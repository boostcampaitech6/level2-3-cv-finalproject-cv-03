...
def capture_frames(cctv_info, buffer, stop_flag, interval=0.1, save_interval=2*60*60, max_save_time=5*60):
    ...
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    output_path = os.path.join(output_dir, f"{time.strftime('%Y%m%d_%H%M%S')}.mp4")
    ...
    while not stop_flag.get():
        if len(buffer) >= frame_per_pred:
            ...
            if len(pred_queue) >= queue_limit:
                if sum(pred_queue) >= anomaly_limit and sum(prob_queue)/sum(pred_queue) > threshold:
...
def inference(
    cctv_info, capture_interval=0.1, frame_per_sec=10, total_sec=3
):
    ...
    predict_thread = threading.Thread(
        target=predict,
        args=(cctv_info['cctv_id'], model, buffer, stop_flag, frame_per_sec, total_sec),
    )
    ...
    threshold, save_time_length, temp_frames = cctv_info['threshold'], cctv_info['save_time_length'], []
    ...
    while True:
        k, v = redis_server.brpop(keys=[flag_key, alarm_key, anomaly_key], timeout=None)
        ...
        if k == flag_key:
            ...
            if not os.path.isdir(os.path.dirname(anomaly_save_path)): os.makedirs(os.path.dirname(anomaly_save_path), exist_ok=True)
